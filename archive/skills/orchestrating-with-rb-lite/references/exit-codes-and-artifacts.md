...
| `12` | `max_rounds_hit` | Burned all `--max-rounds` without convergence | Inspect the latest review files; either bump `--max-rounds` or address the remaining findings manually. Do **not** raise `--min-findings-severity` to skip nits — it silences the defect reviewers' P2s, where real should-fix findings live |
