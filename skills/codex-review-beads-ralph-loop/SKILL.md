...
| Tool | Resolution |
|------|------------|
| `codex` | Must be on `PATH` and authenticated. Default to `-c model="gpt-5.6-sol" -c model_reasoning_effort="xhigh"` for reviews; fall back to env default if `gpt-5.6-sol` is unavailable and note the fallback. Codex auth can break mid-session (401) — see pitfall #8 in references/orchestration-pitfalls.md. |
