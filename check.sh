...
# asked. Reads DRIVE.md and one `git rev-parse` to find the repo root — no forge, no
# network. Outside a checkout it reports no repository and exits 0, so it is a gate for
# this repo rather than a general assertion.
